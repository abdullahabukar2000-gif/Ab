...
// as in a Quran app, and opens at the page where it begins.

import { allChapters, ayahRange, getChapter, surahStartPage, surahsOnPage } from './data';
import { h } from './dom';
...
export function renderHome(lastPage: number | null, actions: HomeActions): HTMLElement {
  ...
  // --- continue reading
  const last = lastPage != null && ayahRange(lastPage) ? lastPage : null;
  if (last != null) {
    const [from, to] = ayahRange(last)!;
    const chapter = getChapter(Number(from.split(':')[0]));
    const card = h('section', { class: 'continue' },
      h('p', { class: 'continue-label' }, 'Continue reading'),
      h('p', { class: 'continue-surah' }, surahsOnPage(last).map((id) => getChapter(id)?.name_complex ?? '').join(' · ')),
      h('p', { class: 'continue-where' }, `Page ${last} · ${from}–${from.split(':')[0] === to.split(':')[0] ? to.split(':')[1] : to}`),
...
function surahRow(id: number, actions: HomeActions): HTMLElement {
  ...
  const start = surahStartPage(id);
  const status = `${c.verses_count} ayahs · page ${start}`;

  const row = h('button', { type: 'button', class: 'surah' });
